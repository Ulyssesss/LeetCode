...
public class _0133 {
    ...
    private Node cloneGraphHelper(Node node, Map<Node, Node> nodeMap) {
        Node cloneNode = nodeMap.get(node);
        if (cloneNode == null) {
            cloneNode = new Node(node.val);
            nodeMap.put(node, cloneNode);
            for (Node neighbor : node.neighbors) {
                cloneNode.neighbors.add(cloneGraphHelper(neighbor, nodeMap));
            }
        }
        return cloneNode;
    }
